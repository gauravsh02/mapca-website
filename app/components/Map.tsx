...
const Map: React.FC<IMap> = ({ mapType, mapTypeControl = false, setDistanceInKm}) => {
  ...
  const startMap = (): void => {
    if (!map) {
      ...
      const homeLocation = new google.maps.LatLng(28.577726,77.23029);
  ...
  const defaultMapStart = (): void => {
    const defaultAddress = new google.maps.LatLng(28.577726,77.23029);
  ...
  const addHomeMarker = (location: GoogleLatLng): GoogleMarker => {
    const homeMarkerConst:GoogleMarker = new google.maps.Marker({
      ...
      map: map,
      icon: {
        url: window.location.origin + '/homeAddressMarker.png'
      }
    });
    ...
    homeMarkerConst.addListener('click', () => {
      map?.panTo(location);
      // map?.setZoom(6);
      infowindow.open(map, homeMarkerConst); 
    });
